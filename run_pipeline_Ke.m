...
files    = dir(fullfile(foldername,['*.',filetype]));
...
tic
[A,b,C,f,S,P,RESULTS,YrA] = run_CNMF_patches(data,K,patches,tau,p,options);  % do not perform deconvolution here since
toc                                                                             % we are operating on downsampled data
