...
name = 'RVKC438_2019-08-17.avi';
tic
fprintf('Loading data...\n')
Yr = read_file(name);
fprintf('Finish loading data, cost %f s',toc)
[d1,d2,T] = size(Yr);
tic
Y_ds = downsample_data(Yr,'time',2); % downsampled the data
Y_ds = single(Y_ds);
fprintf('Finish downsampling data in %f', toc)
...
tic; Mr = apply_shifts(h5_name_down,shifts1,options_r); toc % apply shifts to full dataset
