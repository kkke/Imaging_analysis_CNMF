function resp = stats_2p_overlapTasteLick(trial, overlap)
...
for j = 1:length(taste)
    for i = 1:length(overlap)
        ...
        for kkk = 1:length(temp_taste)
            ...
            tasteTest(kkk) = mean(full(trial(temp_taste(kkk)).S_trace(overlap(i),indTest(kkk,:)))); % get 5 frames around the peak.
        end
