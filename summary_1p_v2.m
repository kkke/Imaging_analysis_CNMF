function summary_1p_v2(trial,neuron_data, neuron,Coor)
clear taste
...
taste_all =taste(:,1:5);
taste_all(find(taste_all ==-1))=1; % treat the inhibitory response as excitatory
...
ind_cue  = find(taste(:,6)~=0); % cue response
ind_lick = find(taste(:,7)~=0); % lick response
...
%% plot cue response
ind_cue_inh = find(taste(:,6)==-1); % lick response
...
ind_lick_inh = find(taste(:,7)==-1); % lick response
[~,ia,~] = intersect(ind_lick,ind_lick_inh);
[~,ib] = setdiff(ind_lick,ind_lick_inh);
% figure;
% imagesc(time.lick,[],lick)
% xlim([-2,4])
% caxis([-0.2,1.2])
figure;
boundedline(time.lick, mean(lick(ia,:)),(std(lick(ia,:))./sqrt(size(lick(ia,:),1)))','alpha','b');
hold on
boundedline(time.lick, mean(lick(ib,:)),(std(lick(ib,:))./sqrt(size(lick(ib,:),1)))','alpha','r');
xlim([-6,4])
xlabel('Time')
ylabel('dF/F')
ylim([-0,0.05])
